...
# fleet-manager re-arms it (fleet-task.sh watch <id>) after handling each event,
# so supervision stays event-driven in any harness (no polling, no harness
# background tasks).
...
# past it -> instant; worker already finished -> 60s, then settle correctly).
herdr agent wait "$agent" --until working --timeout 60000 >/dev/null 2>&1
